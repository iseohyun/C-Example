...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        for (int i = 0; i < MAX_CIRCLE; i++) {
            hBrush = CreateSolidBrush(pos[i].color);
            SelectObject(hdc, hBrush);
            //Ellipse(hdc, p->x - p->r, p->y - p->r, p->x + p->r, p->y + p->r);p++;
            Ellipse(hdc, pos[i].x - pos[i].r, pos[i].y - pos[i].r, pos[i].x + pos[i].r, pos[i].y + pos[i].r);
            DeleteObject(hBrush);
        }
    ...
    case WM_CREATE:
        ...
        for (int i = 0; i < MAX_CIRCLE; i++) {
            CreateCircle(pos + i, r.right, r.bottom);
        }
